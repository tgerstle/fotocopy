...
import { consolidateComponents } from "../../scripts/llm/consolidator";
  ...
  beforeAll(async () => {
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        title: "Page 1",
        layout: [
          { blockType: "HeroBanner", data: { title: "Hello", subtitle: "World" } },
          { blockType: "StaffGrid", data: { teamMembers: "array" } }
        ]
      })
    );
    ...
    await fs.writeFile(
      ...
      JSON.stringify({
        title: "Page 2",
        layout: [
          { blockType: "HeroBanner", data: { title: "Hi", image: "img.jpg", designTokens: {} } },
          { blockType: "FAQ", data: { questions: "q" } }
        ]
      })
    );
  });
