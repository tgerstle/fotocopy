...
Instead of just mocking the final CMS data, we will manually create mock outputs for _every stage_ of the pipeline. We then write minimal Node scripts to transform these mocks from one step to the next, proving that the data flow (from raw CSS tokens to React components) is computationally unbroken.
