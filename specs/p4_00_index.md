...
- Instructions to use Next.js best practices (e.g., `next/image`).

### 3. The Developer Q/A Loop

- An engineer opens the generated prompt and uses native VS Code Copilot Chat to execute it.
...
- Adds a step where we automatically generate a `.prompt.md` for our mock `Hero` block based on the `CMSBlockSchema` we defined in `p0_03_hydrator.md`.
