...
- [ ] Create `scripts/intersection/orchestrator.ts` to wire inputs and outputs together.
- [ ] Implement physical deletion of `nodeMap.json` nodes based on `globalHashes`.
- [ ] Refactor `template_inference.ts` to accept and utilize actual hash arrays instead of hardcoded semantic tag removal.

## Verification & Tests

- [ ] **E2E Integration Pass:** Add a Vitest suite feeding mathematical mocks through the Orchestrator, proving that global nodes are automatically omitted from both the generated sequential chunks and the clustering inference logic.