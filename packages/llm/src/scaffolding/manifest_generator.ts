...
export async function generateManifests(
  ...
  for (const [pageSlug, chunks] of Object.entries(pages)) {
    ...
    const tokensFilename = `${cleanSlug}__tokens.json`;
    
    let domPath = path.join(liveCaptureDir, domFilename);
    const tokensPath = tempTokensPathCheck(liveCaptureDir, tokensFilename, cleanSlug);

    let domExists = true;
    try { await fs.access(domPath); } catch { 
      domPath = path.join(liveCaptureDir, `${cleanSlug}_dom.html`); // check single underscore
      try { await fs.access(domPath); } catch { domExists = false; }
    }
    ...
    const tempLlmPath = path.join(outputManifestsDir, `_temp_${cleanSlug}.json`);
    ...
    try {
        ...
        const manifestOutPath = path.join(outputManifestsDir, `${cleanSlug}.json`);
        await fs.writeFile(manifestOutPath, JSON.stringify(hydratedData, null, 2));
...
function tempTokensPathCheck(dir: string, expected: string, slug: string) {
