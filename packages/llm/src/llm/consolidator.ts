    ...
    try {
      ...
      if (data && data.layout && Array.isArray(data.layout)) {
        for (const block of data.layout) {
          const type = block.blockType || block.inferredBlockType;
          if (type && type !== "N/A" && type !== "Unknown") {
            if (!sampleData[type]) {
              sampleData[type] = block.data || block.mappings || {};
            }
          }
        }
      }
    } catch (e) {
      console.warn(`Could not read or parse ${file} for sample data:`, e);
    }
  }
