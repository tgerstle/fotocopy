...
async function run() {
  ...
  if (targetStep === 0 || targetStep === 1) {
    ...
    if (
      ...
      fotocopyConfig.staticUrlList.length > 0
    ) {
      console.log(
        `Bypassing sitemap. Using static URL list (${fotocopyConfig.staticUrlList.length} URLs)...`,
      );
      await fs.writeFile(csvPath, fotocopyConfig.staticUrlList.join("\n"));
    } else if (fotocopyConfig.sitemapUrl) {
      console.log(
        `Fetching dynamic sitemap from ${fotocopyConfig.sitemapUrl}...`,
      );
  ...
  if (targetStep === 0 || targetStep === 2) {
    ...
    try {
      console.log(
        `Executing Purge and Slice geometry against ${outputCaptureDir}...`,
      );
      await executePurgeAndSlice(outputCaptureDir, outputCaptureDir, chunksDir);
    } catch (e) {
      console.warn(
        "Phase 2 Orchestrator encountered an issue (mocking fallback):",
        e,
      );
    }
    ...
    try {
      ...
      console.log(
        "No chunks found. Creating a synthetic chunk for downstream...",
      );
      await fs.mkdir(chunksDir, { recursive: true });
      await fs.writeFile(
        path.join(chunksDir, "synthetic_chunk.json"),
        JSON.stringify({
          id: "synthetic",
          markup: "<div data-awa-id='1'><h2>Mock Title</h2></div>",
        }),
      );
    }
  }
  ...
  const classificationOutput = path.join(
    __dirname,
    "../output/hydration/llm_raw_map.json",
  );
  ...
  if (targetStep === 0 || targetStep === 3) {
    ...
    await consolidateComponents(
      path.join(__dirname, "../output/hydration"),
      manifestPath,
    );
  }
  ...
  if (targetStep === 0 || targetStep === 4) {
    ...
    try {
      ...
      console.warn(
        "Unable to execute template generation due to missing tokens or manifest.",
        e,
      );
    }
  }
  ...
  if (targetStep === 0 || targetStep === 5) {
    ...
    const globalsResultPath = path.join(
      __dirname,
      "../output/hydration/llm_globals_map.json",
    );
    ...
    try {
      ...
      console.log(
        `Queueing global footprints for Singleton shell block discovery...`,
      );
      ...
      const publicAssetsDir = path.join(
        __dirname,
        "../../../packages/sandbox/public/assets",
      );
      await downloadAssetsLocally(
        manifestOutDir,
        publicAssetsDir,
        fotocopyConfig.testTargetUrl || "",
      );
    } catch (e) {
      console.log(
        "Could not process globals. Ensure globals_manifest.json exists.",
        e,
      );
    }
  }
